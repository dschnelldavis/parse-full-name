exports.parseFullName = function parseFullName(
  ...
  if (stopOnError === 1) stopOnError = true
  stopOnError = stopOnError && stopOnError === true ? true : false
    // false = output warnings on parse error, but don't stop
  if (useLongLists === 1) useLongLists = true
  useLongLists = useLongLists && useLongLists === true ? true : false
    // false = use short lists
  ...
  partFound = (' '+nameToParse+' ').match(/\s[‘’']([^‘’']+)[‘’'],?\s/g)
